...
export function registerRefreshCommand(
  ...
  return commands.registerCommand('opencodeGoPacer.refresh', async () => {
    ...
    try {
      ...
      if (err instanceof CredentialsError) {
        await window.showErrorMessage('OpenCode Go Pacer credentials are missing. Please re-configure.', 'Configure Now').then((action) => {
          if (action === 'Configure Now') {
            commands.executeCommand('opencodeGoPacer.configure');
          }
        });
      } else if (t) {
