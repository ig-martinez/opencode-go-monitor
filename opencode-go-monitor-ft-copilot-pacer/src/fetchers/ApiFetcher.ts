...
export class ApiFetcher implements QuotaFetcher {
  ...
  private normalizeResponse(data: Record<string, unknown>): QuotaSnapshot {
    ...
    const addPeriodBoundaries = (
      base: ReturnType<typeof extractWindow>,
      windowType: 'rolling' | 'weekly' | 'monthly',
    ): QuotaSnapshot['rolling'] => {
      const now = new Date();
      const { start, end } = computePeriodBoundaries(windowType, now);
      return {
        ...base,
        periodStartSeconds: start,
    ...
    return {
      timestamp: Date.now(),
      rolling: addPeriodBoundaries(extractWindow(data.rolling), 'rolling'),
      weekly: addPeriodBoundaries(extractWindow(data.weekly), 'weekly'),
      monthly: addPeriodBoundaries(extractWindow(data.monthly), 'monthly'),
      source: 'api',
    };
  }
}
