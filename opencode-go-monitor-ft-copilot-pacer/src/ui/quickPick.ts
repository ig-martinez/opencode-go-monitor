...
export async function showDetailQuickPick(
  ...
  const rollingPace = calculatePacing(snapshot.rolling.usagePercent, snapshot.rolling.periodStartSeconds, snapshot.rolling.periodEndSeconds);
  const weeklyPace = calculatePacing(snapshot.weekly.usagePercent, snapshot.weekly.periodStartSeconds, snapshot.weekly.periodEndSeconds);
  const monthlyPace = calculatePacing(snapshot.monthly.usagePercent, snapshot.monthly.periodStartSeconds, snapshot.monthly.periodEndSeconds);

  qp.items = [
    {
      label: `$(clock) ${rollingPace.progressBar}  ${rollingLabel}: ${formatPercent(snapshot.rolling.usagePercent)} · ${t?.statusBarReset(formatTime(snapshot.rolling.resetsInSeconds)) ?? `resets in ${formatTime(snapshot.rolling.resetsInSeconds)}`}`,
    },
    {
      label: `$(calendar) ${weeklyPace.progressBar}  ${weeklyLabel}: ${formatPercent(snapshot.weekly.usagePercent)} · ${t?.statusBarReset(formatTime(snapshot.weekly.resetsInSeconds)) ?? `resets in ${formatTime(snapshot.weekly.resetsInSeconds)}`}`,
    },
    {
      label: `$(calendar) ${monthlyPace.progressBar}  ${monthlyLabel}: ${formatPercent(snapshot.monthly.usagePercent)} · ${t?.statusBarReset(formatTime(snapshot.monthly.resetsInSeconds)) ?? `resets in ${formatTime(snapshot.monthly.resetsInSeconds)}`}`,
    },
    {
      label: prediction
    ...
    {
      label: '$(link-external) Open OpenCode Pacer dashboard',
    },
    {
