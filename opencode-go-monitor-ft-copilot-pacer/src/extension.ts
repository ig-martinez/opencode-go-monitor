...
function getConfig(): vscode.WorkspaceConfiguration {
  return vscode.workspace.getConfiguration('opencodeGoPacer');
}
...
function createStatusBarItemAdapter(alignment: number, priority: number): import('./ui/statusBar').StatusBarItem {
  const item = vscode.window.createStatusBarItem('opencodeGoPacer', alignment as vscode.StatusBarAlignment, priority);
  return {
    get text() {
      return item.text;
    },
...
export async function activate(context: vscode.ExtensionContext): Promise<vscode.Disposable> {
  ...
  const outputChannel = vscode.window.createOutputChannel('OpenCode Go Pacer');
  context.subscriptions.push(outputChannel);
  outputChannel.appendLine('=== OpenCode Go Pacer extension activated ===');
  outputChannel.appendLine(`VSCode version: ${vscode.version}`);
  outputChannel.appendLine(`Extension path: ${context.extensionPath}`);

  // Expose debug function globally for fetchers to use
  globalThis.opencodeGoPacerDebug = (msg: string) => {
  ...
  async function doFetch(): Promise<void> {
    ...
    try {
      ...
      if (err instanceof CredentialsError) {
        ...
        vscode.window.showErrorMessage('OpenCode Go Pacer credentials are missing. Please re-configure.', 'Configure Now').then((action) => {
          if (action === 'Configure Now') {
            vscode.commands.executeCommand('opencodeGoPacer.configure');
          }
        });
      } else {
  ...
  registered.push(
    vscode.commands.registerCommand('opencodeGoPacer.statusBarClick', async () => {
      statusBarClickCount++;
      ...
      if (statusBarClickCount === 1) {
        ...
        statusBarClickTimer = setTimeout(async () => {
          ...
          await vscode.commands.executeCommand('opencodeGoPacer.refresh');
        ...
        await vscode.commands.executeCommand('opencodeGoPacer.showDetails');
      }
    })
  );
  ...
  const configDisposable = vscode.workspace.onDidChangeConfiguration((e) => {
    if (e.affectsConfiguration('opencodeGoPacer.pollIntervalSeconds')) {
      scheduleNext();
    }
    if (e.affectsConfiguration('opencodeGoPacer.displayWindow')) {
